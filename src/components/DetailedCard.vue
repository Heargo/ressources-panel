<template>
    <div class="glass flex-col" @click="closeCard">
        <div @mouseover="hoverCard=true" @mouseout="hoverCard=false" class="card flex-col">
            <div class="flex-row w-100">
                <img :src="('https://www.google.com/s2/favicons?domain='+item.url+'&sz=128')" alt="icon">
                <h2>{{ item.name }}</h2>
            </div>
            <div class="tags flex-row">
                <p v-for="(tag, index) in item.tags" :key="index" v-show="tag!=''" :class="{'removable':store.editMode,'plannedToRemoved':InRemovedList(index)}" @click="removeTagToggle(index)">{{tag}}</p>
            </div>
