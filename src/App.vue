<template>
  <!-- BACKGROUND -->
  <img
    :src="require(`@/assets/${store.userTheme}/circle-scatter-haikei.svg`)"
    alt="bg"
    class="background"
  />
...
//on load set user theme
store.setTheme(store.userTheme);

store.LoadContent(auth.client);

//every 10s check if the content as been updated locally. If so, save to server
setInterval(() => {
  if (store.UnsavedChanges() && auth.IsConnected && auth.autoSave) {
